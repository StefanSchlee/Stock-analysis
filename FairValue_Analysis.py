...
ax.bar(pd.Timestamp(currentYearEarningEstimateDict['endDate']),
       currentYearEarningEstimateDict['earningsEstimate']['avg'], width=pd.Timedelta(days=30), label="Estimate +0y")
ax.bar(pd.Timestamp(nextYearEarningsEstimateDict['endDate']),
       nextYearEarningsEstimateDict['earningsEstimate']['avg'], width=pd.Timedelta(days=30), label='Estimate +1y')
...
ax.bar(CashFlowSeries.index, CashFlowSeries.array, width=pd.Timedelta(days=30), label="Historic Operative Cashflow")
...
fairValueDateFine = np.linspace(fairValue.index.values[0].astype('float'),
                               fairValue.index.values[-1].astype('float'), 1000)
